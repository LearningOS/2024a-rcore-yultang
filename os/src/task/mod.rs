...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {    // static ref: 全局引用, 智能指针管理
...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        if task0.begin == false {
            task0.begin = true;
            task0.syscall_times = [0u32; MAX_SYSCALL_NUM];
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].begin == false{
                ...
                inner.tasks[next].syscall_times = [0u32; MAX_SYSCALL_NUM];
            }
    ...
    /// Increase the syscall_times depend on the syscall_id
    ...
    fn get_syscall_times(&self, syscall_times: &mut [u32; MAX_SYSCALL_NUM]) {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
...
/// Get current task syscall_times record
pub fn get_syscall_times(syscall_times: &mut [u32; MAX_SYSCALL_NUM]) {
    TASK_MANAGER.get_syscall_times(syscall_times);
}
